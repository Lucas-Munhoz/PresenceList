...
CREATE TABLE IF NOT EXISTS `PresenceListDB`.`PROFESSOR` (
  ...
  PRIMARY KEY (`idProf`, `WORKSHOP_idWork`),
  UNIQUE INDEX `emailProf_UNIQUE` (`emailProf` ASC) VISIBLE,
  UNIQUE INDEX `idPROFESSOR_UNIQUE` (`idProf` ASC) VISIBLE,
  INDEX `fk_PROFESSOR_WORKSHOP1_idx` (`WORKSHOP_idWork` ASC) VISIBLE,
  CONSTRAINT `fk_PROFESSOR_WORKSHOP1`
    FOREIGN KEY (`WORKSHOP_idWork`)
    REFERENCES `PresenceListDB`.`WORKSHOP` (`idWork`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `PresenceListDB`.`LISTA_PRESENCA` (
  ...
  PRIMARY KEY (`WORKSHOP_idWork`, `ALUNOS_raAlun`),
  INDEX `fk_WORKSHOP_has_ALUNOS_ALUNOS1_idx` (`ALUNOS_raAlun` ASC) VISIBLE,
  INDEX `fk_WORKSHOP_has_ALUNOS_WORKSHOP1_idx` (`WORKSHOP_idWork` ASC) VISIBLE,
  CONSTRAINT `fk_WORKSHOP_has_ALUNOS_WORKSHOP1`
    FOREIGN KEY (`WORKSHOP_idWork`)
    REFERENCES `PresenceListDB`.`WORKSHOP` (`idWork`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
